...
unsafe fn insert_node(mut p: *mut Ppmd8, mut node: *mut u8, mut index: u32) {
...
unsafe fn remove_node(mut p: *mut Ppmd8, mut index: u32) -> *mut u8 {
...
unsafe fn split_block(mut p: *mut Ppmd8, mut ptr: *mut u8, mut oldindex: u32, mut newindex: u32) {
    let mut i: u32 = 0;
    let mut nu: u32 = ((*p).index2units[oldindex as usize] as u32)
    ...
    if (*p).index2units[i as usize] as u32 != nu {
        i = i.wrapping_sub(1);
        let mut k: u32 = (*p).index2units[i as usize] as u32;
        insert_node(
            p,
...
unsafe fn glue_free_blocks(mut p: *mut Ppmd8) {
    ...
    while i < PPMD_NUM_INDEXES {
        ...
        while next != 0 as i32 as u32 {
            let mut node: *mut Node = ((*p).base).offset(next as isize) as *mut u8 as *mut Node;
    ...
    *prev = 0 as i32 as u32;
    while n != 0 as i32 as u32 {
        ...
        if (*p).index2units[i_0 as usize] as u32 != nu_0 {
            ...
            let mut k: u32 = (*p).index2units[i_0 as usize] as u32;
            insert_node(
                p,
...
#[inline(never)]
unsafe fn alloc_units_rare(mut p: *mut Ppmd8, mut index: u32) -> *mut u8 {
    let mut i: u32 = 0;
    ...
    let mut block: *mut u8 = remove_node(p, i);
    split_block(p, block, i, index);
    block
}

unsafe fn alloc_units(mut p: *mut Ppmd8, mut index: u32) -> *mut u8 {
    if (*p).free_list[index as usize] != 0 as i32 as u32 {
        return remove_node(p, index);
    }
    let mut numBytes: u32 = (*p).index2units[index as usize] as u32 * 12 as i32 as u32;
...
unsafe fn shrink_units(
    ...
    if (*p).free_list[i1 as usize] != 0 as i32 as u32 {
        let mut ptr: *mut u8 = remove_node(p, i1);
...
unsafe fn free_units(mut p: *mut Ppmd8, mut ptr: *mut u8, mut nu: u32) {
    insert_node(
        p,
        ptr,
...
unsafe fn special_free_unit(mut p: *mut Ppmd8, mut ptr: *mut u8) {
...
unsafe fn expand_text_area(mut p: *mut Ppmd8) {
    ...
    while (*node).stamp == 0xffffffff as u32 {
        ...
        count[(*p).units2index[(nu as usize).wrapping_sub(1 as i32 as usize) as usize] as usize] =
...
unsafe fn restart_model(mut p: *mut Ppmd8) {
    ...
    let mut mc: *mut Context = (*p).hi_unit as *mut u8 as *mut Context;
    ...
    while m < 25 as i32 as u32 {
        ...
        while k < 8 as i32 as u32 {
            ...
            let val: u16 = (((1 as i32) << 7 as i32 + 7 as i32) as u32).wrapping_sub(
                (K_INIT_BIN_ESC[k as usize] as u32).wrapping_div(i.wrapping_add(1 as i32 as u32)),
            ) as u16;
            r = 0 as i32 as u32;
...
pub unsafe fn init(mut p: *mut Ppmd8, mut maxOrder: u32, mut restoreMethod: u32) {
    (*p).max_order = maxOrder;
    (*p).restore_method = restoreMethod;
    restart_model(p);
}

unsafe fn refresh(mut p: *mut Ppmd8, mut ctx: *mut Context, mut oldNU: u32, mut scale: u32) {
...
unsafe fn cut_off(mut p: *mut Ppmd8, mut ctx: *mut Context, mut order: u32) -> u32 {
    ...
    {
        let mut ptr: *mut u8 = remove_node(p, index);
    ...
    loop {
        ...
        if (((*p).base).offset(successor_0 as isize) as *mut u8 as *mut u8) < (*p).units_start {
            let fresh1 = ns;
            ns = ns - 1;
            let mut s2: *mut State = stats.offset(fresh1 as u32 as isize);
            if order != 0 {
                if s_0 != s2 {
                    *s_0 = *s2;
                }
    ...
    if ns != (*ctx).num_stats as i32 && order != 0 {
        ...
        if ns == 0 as i32 {
            ...
            refresh(
                ...
                ctx,
                nu,
                ((*ctx).union2.summ_freq as u32 > (16 as i32 as u32).wrapping_mul(ns as u32)) as i32
                    as u32,
            );
        }
    }
...
unsafe fn restore_model(mut p: *mut Ppmd8, mut ctxError: *mut Context) {
    ...
    if (*p).restore_method == PPMD8_RESTORE_METHOD_RESTART as i32 as u32
        || get_used_memory(p) < (*p).size >> 1 as i32
    {
        ...
        while (*(*p).max_context).suffix != 0 {
            (*p).max_context =
                ((*p).base).offset((*(*p).max_context).suffix as isize) as *mut u8 as *mut Context;
        }
        loop {
...
pub unsafe fn update_model(mut p: *mut Ppmd8) {
    ...
    let mut s0: u32 = 0;
    let mut ns: u32 = 0;
    let mut fFreq: u32 = (*(*p).found_state).freq as u32;
    let mut flag: u8 = 0;
    let mut fSymbol: u8 = (*(*p).found_state).symbol;
    let mut s: *mut State = 0 as *mut State;
    if ((*(*p).found_state).freq as i32) < 124 as i32 / 4 as i32
    ...
    if (*p).order_fall == 0 as i32 as u32 && minSuccessor != 0 {
        let mut cs: *mut Context = create_successors(p, 1 as i32, s, (*p).min_context);
        if cs.is_null() {
    ...
    *fresh4 = (*(*p).found_state).symbol;
    ...
    } else if (((*p).base).offset(minSuccessor as isize) as *mut u8 as *mut u8) < (*p).units_start {
        let mut cs_1: *mut Context = create_successors(p, 0 as i32, s, (*p).min_context);
        if cs_1.is_null() {
            restore_model(p, c);
            return;
        }
    ...
    if (*p).order_fall == 0 as i32 as u32 {
        ...
        (*p).text = ((*p).text).offset(-(((*p).max_context != (*p).min_context) as i32 as isize));
    }
    ...
    while c != (*p).min_context {
        ...
        if ns1 != 0 as i32 as u32 {
            if ns1 & 1 as i32 as u32 != 0 as i32 as u32 {
                ...
                if i != (*p).units2index[(oldNU as usize)
                    ...
                    as usize] as u32
                {
                    let mut ptr: *mut u8 = alloc_units(p, i.wrapping_add(1 as i32 as u32));
                    let mut oldPtr: *mut u8 = 0 as *mut u8;
                    if ptr.is_null() {
                        restore_model(p, c);
                        return;
                    }
                    oldPtr = ((*p).base).offset((*c).union4.stats as isize) as *mut u8 as *mut State
                        as *mut u8;
                    let mut d: *mut u32 = ptr as *mut u32;
                    let mut z: *const u32 = oldPtr as *const u32;
                    let mut n: u32 = oldNU;
            ...
            let mut s_0: *mut State = alloc_units(p, 0 as i32 as u32) as *mut State;
            if s_0.is_null() {
                restore_model(p, c);
                return;
            }
        ...
        let mut s_1: *mut State = (((*p).base).offset((*c).union4.stats as isize) as *mut u8
            as *mut State)
            .offset(ns1 as isize)
...
unsafe fn rescale(mut p: *mut Ppmd8) {
    ...
    (*p).found_state = ((*p).base).offset((*mc_0).union4.stats as isize) as *mut u8 as *mut State;
}
...
pub unsafe fn make_esc_freq(
    ...
    let mut numStats: u32 = (*mc).num_stats as u32;
    if numStats != 0xff as i32 as u32 {
        see =
            ((*p).see[((*p).ns2index[(numStats as usize).wrapping_add(2 as i32 as usize) as usize]
                as u32 as usize)
...
pub unsafe fn update1_0(mut p: *mut Ppmd8) {
    ...
    let mut freq: u32 = (*s).freq as u32;
    let summFreq: u32 = (*mc).union2.summ_freq as u32;
    (*p).prev_success = ((2 as i32 as u32).wrapping_mul(freq) >= summFreq) as i32 as u32;
    (*p).run_length += (*p).prev_success as i32;
    (*mc).union2.summ_freq = summFreq.wrapping_add(4 as i32 as u32) as u16;
    freq = freq.wrapping_add(4 as i32 as u32);
    (*s).freq = freq as u8;
    if freq > 124 as i32 as u32 {
        rescale(p);
