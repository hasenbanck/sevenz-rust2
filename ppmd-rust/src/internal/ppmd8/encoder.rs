        ...
        unsafe {
            ...
            if (*self.min_context).num_stats != 0 {
                ...
                let prob =
                    &mut *(*self
                        ...
                        .offset(*self.ns2index.as_mut_ptr().offset(
                            (((*self.min_context).data.single_state.freq as usize) - 1) as isize,
                        ) as isize))
                    .as_mut_ptr()
                    .offset(
                        self.prev_success
                    ...
                    .cast::<u16>();
                let s = addr_of_mut!((*self.min_context).data.single_state);
                ...
                if (*s).symbol as i32 == symbol {
                    *prob = pr.wrapping_add((1 << 7) as u32) as u16;
                    ...
                    self.prev_success = 1;
                    self.run_length += 1;
                    (*s).freq = (freq + ((freq < 196) as u32)) as u8;
            ...
            loop {
                ...
                loop {
                    ...
                    mc = self.ptr_of_offset((*mc).suffix).cast::<Context>();
                ...
                let mut s = self
                    ...
                    .cast::<State>();
                let mut sum = 0u32;
                let mut i = ((*self.min_context).num_stats as u32) + 1;
                loop {
                    let cur = (*s).symbol as u32;
                    if cur as i32 == symbol {
                        ...
                        if num2 != 0 {
                            ...
                            loop {
                                ...
                                s = s.offset(2);
                                sum +=
                                    (*s.offset(-2)).freq as u32 & char_mask[sym0 as usize] as u32;
                                sum +=
                                    (*s.offset(-1)).freq as u32 & char_mask[sym1 as usize] as u32;
                                num2 -= 1;
                                if !(num2 != 0) {
                                    break;
                                }
                    ...
                    sum += (*s).freq as u32 & char_mask[cur as usize] as u32;
                    s = s.offset(1);
                    i = i.wrapping_sub(1);
                    if !(i != 0) {
                        break;
